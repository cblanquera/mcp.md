...
export const info = {
  ...
  description: 'Sanity-check topics and tags before running a search; suggest alternatives for missing items.',
  inputSchema: schema
};

export function register(server: McpServer, store: JsonlStore) {
  server.registerTool('validate_filters', info, async args => {
    //get zod params
