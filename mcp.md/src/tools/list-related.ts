...
export const schema = {
  ...
  strategy: z.enum(['tags', 'document']).optional().describe(
...
export const info = {
  title: 'List Related',
  description: 'Find related sections by shared tags or by '
    + 'same-document neighborhood.',
  inputSchema: schema
};

export async function handler(args: Args, store: JsonlStore) {
  //extract variables from zod params
  const { id, strategy = 'tags', limit = 10 } = z.object(schema).parse(args);
...
export function register(server: McpServer, store: JsonlStore) {
  server.registerTool('list_related', info, args => handler(args, store));
};
