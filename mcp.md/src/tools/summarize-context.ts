...
export const schema = {
  ...
  max_chars: z.number().int().min(500).max(200000).default(12000).describe(
    `Soft cap on returned characters; tool will truncate at paragraph boundaries.`
  ),
  strip_code: z.boolean().default(false).describe(
    `If true, remove fenced code blocks from the returned context.`
  ),
  include_meta: z.boolean().default(true).describe(
...
export const info = {
  ...
  description: 'Gather and clean context for the agent to summarize. This tool aggregates text; it does not perform the summary.',
  inputSchema: schema
};

export function register(server: McpServer, store: JsonlStore) {
  server.registerTool('summarize_context', info, async args => {
    //get zod params
